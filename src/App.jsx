  ...
  return (
    <div className="h-screen bg-slate-50 font-sans text-slate-800 flex overflow-hidden">
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden bg-slate-50">
        ...
        {/* ⭐️ 배정결과 리포트 화면 (세부 인터페이스 상시 노출 환원) */}
        {activeTab === 'results' && (
          <div className="space-y-6 flex-1 flex flex-col min-h-0 h-full animate-fade-in">
            ...
            <div className="bg-white rounded-[2.5rem] shadow-2xl border border-slate-100 overflow-hidden flex flex-col flex-1 min-h-0">
               ...
               <div className="flex-1 overflow-auto select-text custom-scrollbar min-h-0 bg-white relative">
                  {results.length === 0 && (
                      <div className="absolute inset-0 flex flex-col justify-center items-center bg-white/70 backdrop-blur-sm z-30 text-slate-500 pointer-events-none">
                          <BarChart3 size={40} className="mb-2 opacity-50 text-indigo-500" />
                          <p className="font-black text-sm">현재 배정된 데이터가 없어 표가 비어있습니다.</p>
                          <p className="text-xs font-bold mt-1">우측 상단의 [자동 배정 실행] 버튼을 누르시면 아래 표에 데이터가 채워집니다.</p>
                      </div>
                  )}
